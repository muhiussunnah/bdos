...
export async function POST(req: Request) {
  ...
  const { projectId, category, area, count = 10 } = await req.json().catch(() => ({}));
  if (!projectId || !category) return bad('projectId and category are required');
  ...
  const { system, user } = discoveryPrompt(project as Project, category, area || 'anywhere', n, knowledge);
  ...
  const rows = (result.leads || []).slice(0, n).map((l) => {
    ...
    return {
      ...
      website: l.website || null, industry: l.industry || null, location: l.location || area || null,
